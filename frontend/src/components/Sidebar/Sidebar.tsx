...
export function Sidebar() {
  ...
  // Grouped so related destinations sit together instead of one flat list of 9:
  // workspaces (where you do things) → insights (what happened) → utility.
  const navGroups = [
    [
      { path: '/', icon: MessageSquare, label: 'Chat' },
      { path: '/studio', icon: SquareTerminal, label: 'Studio' },
    ...
    <>
      ...
      >
        <div className="flex flex-col h-full w-[260px]">
          ...
          {/* Bottom nav — grouped with subtle dividers */}
          <nav className="px-2 pb-3 pt-2" style={{ borderTop: '1px solid var(--color-border)' }}>
            {navGroups.map((group, gi) => (
              <div
                key={gi}
                className="flex flex-col gap-0.5 py-1"
                style={gi > 0 ? { borderTop: '1px solid var(--color-border-subtle)' } : undefined}
              >
                {group.map((item) => {
