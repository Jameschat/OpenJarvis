...
DEFAULT_AGENTS: List[Dict[str, Any]] = [
    ...
    },
    {
        ...
        "role": (
            ...
            "memos.\n\n"
            "You ALSO have the Anthropic claude-for-financial-services plugins "
            "installed (2026-05-09):\n"
            "- Skill bundles: `financial-analysis` (DCF/comps/LBO/3-statement/"
            "audit-xls/competitive-analysis), `equity-research` (earnings-analysis/"
            "earnings-preview/initiating-coverage/morning-note/sector-overview/"
            "thesis-tracker/idea-generation/catalyst-calendar/model-update). "
            "Invoke via the Skill tool when the task fits.\n"
            "- Named subagents: `earnings-reviewer` (earnings call + filings → "
            "model update + note draft) and `market-researcher` (sector/theme → "
            "industry overview + competitive landscape). Task-dispatch them "
            "directly when the operator asks for that shape of output.\n"
            "- Slash commands wired by the plugins: /dcf, /comps, /lbo, "
            "/3-statement-model, /earnings, /earnings-preview, /morning-note, "
            "/initiate, /screen, /sector, /thesis, /catalysts, /model-update.\n\n"
            "Routing heuristic: equities/earnings/sector questions → "
            "equity-research skills or earnings-reviewer/market-researcher "
            "subagents. Modelling work (DCF/comps/LBO) → financial-analysis "
            "skills. Crypto, paper-trading, and the daily 06:15 markets brief "
            "are handled by the in-house `financial-researcher` agent and "
            "`markets/` module — do NOT delegate those upstream; they have "
            "their own data path (CoinGecko + yfinance via "
            "C:/Users/User/.openjarvis/markets/markets.db)."
        ),
        "model": _CLAUDE_MODEL or _ARCHITECT_MODEL,
...
def _run_qwen_task(task: Task, agent_spec: Dict[str, Any]) -> None:
    ...
    prompt = "\n\n".join(
        part for part in (
            ...
            "status, non-secret repo inspection, local browser visual checks, "
            "installed Jarvis skills, and Superpowers workflow guidance. "
