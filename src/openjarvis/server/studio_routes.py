...
def _maybe_switch_local_lane(prior: str, new: str) -> bool:
    ...
    lane_of = {"local35": "q35", "coder": "coder", "fast": "fast", "gptoss": "gptoss"}
...
async def studio_qwen_profile_set(request: Request) -> dict[str, Any]:
    try:
        ...
        if profile not in {"local35", "fast", "quality", "remote", "coder", "gptoss"}:
            raise HTTPException(status_code=400, detail={"error": "profile must be local35, fast, quality, remote, coder, or gptoss"})
        try:
