...
def test_brain_operations_center_promotes_studio_instead_of_jarvis_os():
    html = (ROOT / "jarvis_web" / "brain.html").read_text(encoding="utf-8")

    assert 'id="ob-jarvis-os-btn"' not in html
    assert "window.location='/jarvis-os'" not in html
    assert ">Jarvis OS<" not in html
    assert 'id="ob-studio-btn"' in html
    assert "window.location='/studio'" in html
    assert ">Studio<" in html


def test_brain_server_resolves_studio_static_file():
    from openjarvis.cli.brain_server import _jarvis_web_path

    assert _jarvis_web_path("studio.html") == ROOT / "jarvis_web" / "studio.html"
    assert _jarvis_web_path("studio.html").exists()


def test_brain_server_jarvis_os_route_redirects_to_studio_shell():
    ...
    assert '"/studio.html"' in route_block
    assert 'self.path = "/studio.html"' in route_block
